...
export default function BacklanOrderPage() {
  const [qty, setQty] = useState(1);
  const [cable, setCable] = useState("");
  const [cableCustom, setCableCustom] = useState("");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body = {
        _subject: "Purchase Inquiry: BACKLAN Table Lamp",
        _replyto: email,
        product: "BACKLAN Table Lamp",
        quantity: qty,
        cable: cable === "custom" ? `Custom: ${cableCustom}` : cable || "Standard",
        name,
        email,
      ...
      <main className="px-5 md:px-[100px] pt-[90px] pb-24 min-h-screen flex flex-col justify-center">
        <div className="max-w-md">
          ...
          <p className="text-[0.78rem] leading-[2] text-[#888] mb-8">
            We have received your inquiry for the BACKLAN Table Lamp. We will get back to you with full details before any payment is made.
          </p>
    ...
    <main className="px-5 md:px-[100px] pt-[90px] pb-24">
      <div className="pt-10 pb-10 border-b border-[#DCDAD5]">
        <p className="text-[0.6rem] tracking-[0.12em] text-[#888] uppercase mb-4">
          ...
          {" / "}
          <Link href="/shop/backlan" className="hover:text-[#2222FF] transition-colors">BACKLAN Lamp</Link>
          {" / "}Inquire
        </p>
      ...
      <form onSubmit={handleSubmit} className="pt-12 flex flex-col gap-14 max-w-2xl">
        ...
        {/* Step 2: Cable customisation */}
        <div>
          <p className="text-[0.58rem] tracking-[0.18em] text-[#2222FF] uppercase mb-6">02 — Cable & filter (optional)</p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              { id: "standard", label: "Standard", sub: "White cable" },
              { id: "black", label: "Black", sub: "Black cable" },
              { id: "custom", label: "Custom", sub: "Your choice" },
            ].map((opt) => (
              <button key={opt.id} type="button"
                onClick={() => { setCable(opt.id); if (opt.id !== "custom") setCableCustom(""); }}
                className={`flex flex-col items-start gap-2 p-5 border rounded-sm transition-all ${
                  cable === opt.id ? "border-[#2222FF] bg-white" : "border-[#DCDAD5] hover:border-[#0A0A0A]"
                }`}>
                <p className={`text-[0.75rem] font-bold tracking-[0.04em] ${cable === opt.id ? "text-[#2222FF]" : "text-[#0A0A0A]"}`}>{opt.label}</p>
                <p className="text-[0.6rem] tracking-[0.08em] text-[#888] uppercase">{opt.sub}</p>
              </button>
            ))}
          </div>
          {cable === "custom" && (
            <div className="mt-5">
              <textarea value={cableCustom} onChange={(e) => setCableCustom(e.target.value)}
                placeholder="Describe your preferred cable colour or light filter…" rows={3}
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6 border-t border-[#DCDAD5] pt-10">
          ...
          <p className="text-[0.58rem] text-[#888] leading-[1.8]">
            Price: ₪260 / €75 per unit. No payment required now — we will confirm all details before any charge is made.
          </p>
        </div>
      </form>
    </main>
