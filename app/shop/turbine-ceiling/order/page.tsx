...
export default function TurbineCeilingOrderPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body = {
        _subject: "Purchase Inquiry: TURBINE Ceiling Speaker",
        _replyto: email,
        product: "TURBINE Ceiling Speaker",
        quantity: qty,
      ...
      <main className="px-5 md:px-[100px] pt-[90px] pb-24 min-h-screen flex flex-col justify-center">
        <div className="max-w-md">
          ...
          <p className="text-[0.78rem] leading-[2] text-[#888] mb-8">
            We have received your inquiry for the TURBINE Ceiling Speaker. We will get back to you with full details — including pricing and delivery — before any payment is made.
          </p>
    ...
    <main className="px-5 md:px-[100px] pt-[90px] pb-24">
      <div className="pt-10 pb-10 border-b border-[#DCDAD5]">
        <p className="text-[0.6rem] tracking-[0.12em] text-[#888] uppercase mb-4">
          ...
          {" / "}
          <Link href="/shop/turbine-ceiling" className="hover:text-[#2222FF] transition-colors">TURBINE Ceiling Speaker</Link>
          {" / "}Inquire
        </p>
      ...
      <form onSubmit={handleSubmit} className="pt-12 flex flex-col gap-14 max-w-2xl">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6 border-t border-[#DCDAD5] pt-10">
          ...
          <p className="text-[0.58rem] text-[#888] leading-[1.8]">
            Price: ₪12,500 / €3,600 per unit. No payment required now — we will confirm all details including delivery before any charge is made.
          </p>
        </div>
      </form>
    </main>
